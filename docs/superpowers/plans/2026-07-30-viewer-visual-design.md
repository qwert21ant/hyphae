...
- **CSS discipline:** single-class selectors only; no element-type selectors; no nesting; no `!important` in the new stylesheets (the one existing `!important` in `Canvas.tsx`'s generated CSS stays, and its comment explains why). Continue the existing BEM-ish naming (`tree-panel__head`). Set padding/margin for an element in exactly one place.

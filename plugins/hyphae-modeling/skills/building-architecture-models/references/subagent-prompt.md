...
3. Create all your Components in one `mcp__hyphae__create_nodes` call (domain values in each item's `fields`), each `parentId` = {{CONTAINER_ID}}, create-or-skip by name. Fill `description`, and put domain values (`responsibilities`, `invariants`, `technology`) in the `fields` bag where known — `describe_profile` (step 0) lists the valid keys.
4. Create all intra-container edges in one `mcp__hyphae__create_connections` call, ONLY when BOTH endpoints are your own Components. Set the connection `type` (a profile connection kind id) and put `transport`/`intent` in the `fields` bag.
